...
def get_alarm(request):
    form, error = JsonParser(
        Argument("type", required=False), Argument("name", required=False)
    ).parse(request.GET, True)
    if error is None:
        now = datetime.now()
        data = {human_date(now - timedelta(days=x + 1)): 0 for x in range(14)}
        for alarm in Alarm.objects.filter(
            status="1", created_at__gt=human_date(now - timedelta(days=14)), **form
        ):
...
def get_request(request):
    form, error = JsonParser(Argument("duration", type=list, help="参数错误")).parse(
        request.body
    )
    if error is None:
        ...
        for req in DeployRequest.objects.filter(
            created_at__gt=s_date, created_at__lt=e_date
        ):
